...
function parseRow(
  ...
  return {
    ...
    intraOpComplications: String(row['intra-op complications'] ?? row['Intra-op Complications'] ?? ''),
    postOpComplications: String(row['post-op complication'] ?? row['Post-op Complications'] ?? ''),
    histology: String(row['Histology'] ?? row['histology'] ?? ''),
    followUp: String(row['follow up'] ?? row['Follow Up'] ?? ''),
