...
describe('useOperations', () => {
  ...
  it('addOperation creates an entry in IndexedDB', async () => {
    ...
    let entry: unknown;
    await act(async () => {
      entry = await result.current.addOperation({
        date: '2025-03-15',
        patientId: 'PT001',
        chemotherapy: '',
  ...
  it('updateOperation modifies the entry and bumps updatedAt', async () => {
    ...
    await act(async () => {
      await result.current.updateOperation(op.id, { diagnosis: 'Updated diagnosis' });
    });
  ...
  it('deleteOperation performs soft delete', async () => {
    ...
    await waitFor(() => {
      expect(result.current.operations.find(o => o.id === op.id)).toBeUndefined();
    });
  });

  it('operations list excludes soft-deleted entries', async () => {
    await db.operations.bulkAdd([
      createOperation({ userId: 'test-user', deleted: false, diagnosis: 'Visible' }),
      createOperation({ userId: 'test-user', deleted: true, diagnosis: 'Deleted' }),
    ]);
  ...
  it('operations are sorted by date descending', async () => {
    await db.operations.bulkAdd([
      createOperation({ userId: 'test-user', date: '2025-01-01', diagnosis: 'Old' }),
      createOperation({ userId: 'test-user', date: '2025-06-01', diagnosis: 'New' }),
      createOperation({ userId: 'test-user', date: '2025-03-15', diagnosis: 'Mid' }),
    ]);
